...
def gps_bearing(lat1, lon1, lat2, lon2):
    ...
    lon1 = radians(lon1)
    lon2 = radians(lon2)
    # dLat = lat2 - lat1  # dLat is assigned but never used
    dLon = lon2 - lon1
    y = sin(dLon) * cos(lat2)
...
class PosVector:
    ...
    def distance(self, pos2):
        if isinstance(pos2, PosLLH):
            pos2 = pos2.ToECEF()
        return math.sqrt(
            (self.X-pos2.X)**2 + (self.Y-pos2.Y)**2 + (self.Z-pos2.Z)**2
        )
    ...
    def SagnacCorrection(self, pos2):
        ...
        on on RTCM2.3 appendix C.
        Note that this is not a symmetric error!
