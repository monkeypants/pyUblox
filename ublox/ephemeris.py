...
class EphemerisData:
    ...
    See IS-GPS-200F.pdf Table 20-III for the field meanings,
    scaling factors and field widths
    '''
    ...
    def __init__(self, msg):
        ...
        # these were assigned but were not use
        ...
        iodc = (
            self.GET_FIELD_U(msg.sf1d[0],  2,  0) << 8
        ) | self.GET_FIELD_U(msg.sf1d[5],  8, 16)
        ...
        m_0 = (
            self.GET_FIELD_S(msg.sf2d[1],  8,  0) << 24
        ) | self.GET_FIELD_U(msg.sf2d[2], 24,  0)
        c_uc = self.GET_FIELD_S(msg.sf2d[3], 16,  8)
        e = (
            self.GET_FIELD_U(msg.sf2d[3],  8,  0) << 24
        ) | self.GET_FIELD_U(msg.sf2d[4], 24,  0)
        c_us = self.GET_FIELD_S(msg.sf2d[5], 16,  8)
        a_powhalf = (
            self.GET_FIELD_U(msg.sf2d[5],  8,  0) << 24
        ) | self.GET_FIELD_U(msg.sf2d[6], 24,  0)
        ...
        omega_0 = (
            self.GET_FIELD_S(msg.sf3d[0],  8,  0) << 24
        ) | self.GET_FIELD_U(msg.sf3d[1], 24,  0)
        c_is = self.GET_FIELD_S(msg.sf3d[2], 16,  8)
        i_0 = (
            self.GET_FIELD_S(msg.sf3d[2],  8,  0) << 24
        ) | self.GET_FIELD_U(msg.sf3d[3], 24,  0)
        c_rc = self.GET_FIELD_S(msg.sf3d[4], 16,  8)
        w = (
            self.GET_FIELD_S(msg.sf3d[4],  8,  0) << 24
        ) | self.GET_FIELD_U(msg.sf3d[5], 24,  0)
    ...
    def __eq__(self, other):
        ...
        '''
        attrs = [
            'svid', 'valid', 'Tgd', 'A', 'cic', 'cis', 'crc',
            'crs', 'cuc', 'cus', 'deltaN', 'ecc', 'i0', 'idot',
            'M0', 'omega', 'omega_dot', 'omega0',
...
class IonosphericData:
    ...
    def __init__(self, msg):
        '''
        parse assuming a subframe 4 page 18 message
        containing ionospheric data
        '''
        words = msg.dwrd
        for i in range(10):
        ...
        # print(
        #     "svid=%u id=%u pageID=%u" % (self.svid, self.id, self.pageID)
        # )
        # '''
        if self.valid:
            print(
                "a0=%g a1=%g a2=%g a3=%g b0=%g b1=%g b2=%g b3=%g leap=%u" % (
